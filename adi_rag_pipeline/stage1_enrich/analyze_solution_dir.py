...
SIGNAL_CHAIN_DESCRIPTIONS_FILENAME = "signal_chain_descriptions.json"
...
def get_llm_client():
    ...
    try:
        ...
        api_key = ""
        key_path = os.path.join(PROJECT_ROOT, "static", "key1.txt")
...
def get_llm_model() -> str:
    try:
        ...
        return LLM_MODEL or "doubao-seed-2-0-lite-260215"
    except Exception:
        return os.getenv("LLM_MODEL", "doubao-seed-2-0-lite-260215")
...
def load_signal_chain_descriptions(path: str) -> str:
    ...
    if not os.path.isfile(path):
        return "(本方案暂无 signal_chain_descriptions.json，图转文描述需另行生成)"
    ...
    if not chains:
        return "(chains 为空)"
    parts = ["【信号链图转文描述 signal_chain_descriptions.json】"]
    for c in chains:
